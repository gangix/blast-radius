...
def test_writeback_roundtrip_live():
    ...
    try:
        ...
        # write via the Kit
        ac.ensure_tag(DEMO_TAG, name="blast-radius-spike-test",
        ...
        # cleanup so no residue, even if an assertion above failed
        ...
        with DataHubContext(ac._client):
            ...
            graph = get_graph()
            if doc_urn and hasattr(graph, "soft_delete_entity"):
                graph.soft_delete_entity(doc_urn)
